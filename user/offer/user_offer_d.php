...
function searchItem(){
	 ...
	 $query = "SELECT `receivers_item`,`offerers_item`, `offerers_id`, `receivers_item` FROM `Offers_declined` WHERE `receivers_id` = '".mysql_real_escape_string($_SESSION['user_id'])."'";
	...
	if(mysql_num_rows($query_run)==0){
		...
		while ($result = mysql_fetch_assoc($query_run)){
			...
			echo "<a href = 'user_decline_offer_d.php?id1=$result4&id2=$result2'><input type = 'button' name = 'Delete'></a>";
			echo"$result4";
			echo"</td>";
